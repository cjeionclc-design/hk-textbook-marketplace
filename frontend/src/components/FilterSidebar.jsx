  ...
  return (
    <div className="neo-card p-5 space-y-5">
      <div className="flex items-center justify-between">
        <span className="font-extrabold text-gray-700 text-sm">🔍 筛选</span>
        {activeCount > 0 && (
          <span className="text-xs text-white px-2.5 py-0.5 rounded-full font-bold" style={{background:'#ff7b3d'}}>{activeCount}</span>
        )}
      </div>

      <div>
        <label className="text-xs font-bold text-gray-400 uppercase tracking-wider">📂 科目</label>
        <select value={filters.category_id || ''}
          onChange={e => onChange({ ...filters, category_id: e.target.value || null })}
          className="w-full neo-inset px-3 py-2.5 mt-1.5 text-sm font-bold text-gray-600">
      ...
      <div>
        <label className="text-xs font-bold text-gray-400 uppercase tracking-wider">🌐 语言</label>
      ...
      <div>
        <label className="text-xs font-bold text-gray-400 uppercase tracking-wider">⭐ 新旧</label>
      ...
      <div>
        <label className="text-xs font-bold text-gray-400 uppercase tracking-wider">💰 价格范围</label>
        <div className="flex gap-2 mt-1.5">
          <input type="number" placeholder="最低" value={filters.min_price || ''}
            onChange={e => onChange({ ...filters, min_price: e.target.value || null })}
            className="w-full neo-inset px-3 py-2.5 text-sm font-bold" />
