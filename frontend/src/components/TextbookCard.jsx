...
export default function TextbookCard({ listing }) {
  ...
  return (
    <Link to={`/listings/${listing.id}`}
      className="block bg-white rounded-xl shadow-sm border hover:shadow-md transition-shadow p-3 sm:p-4 min-w-0">
      <div className="text-xs text-gray-400 mb-1 truncate">{listing.textbook_category_name_zh}</div>
      <h3 className="font-semibold text-gray-900 mb-1 line-clamp-2 text-sm sm:text-base">{listing.textbook_title}</h3>
      ...
      <div className="flex flex-wrap items-baseline gap-x-2 gap-y-0.5 mt-3">
        <span className="text-xl sm:text-2xl font-bold text-red-500">HK${listing.price}</span>
        <span className="text-xs sm:text-sm text-gray-400 line-through">HK${listing.textbook_original_price}</span>
        {discount > 0 && (
          <span className="text-xs bg-green-100 text-green-700 px-1.5 py-0.5 rounded shrink-0">-{discount}%</span>
        )}
      </div>
