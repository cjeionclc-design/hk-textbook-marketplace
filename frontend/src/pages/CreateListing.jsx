...
function CreateListingForm() {
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <h1 className="text-2xl font-bold mb-6">Sell a Textbook</h1>
      {error && <div className="bg-red-100 text-red-700 p-3 rounded mb-4">{error}</div>}

      {step === 1 && (
        <div>
          <p className="text-gray-600 mb-4">Step 1: Find the textbook</p>
            ...
            <div className="space-y-2 mb-4">
              {searchResults.map(t => (
                <button key={t.id}
                  ...
                  className="w-full text-left p-3 border rounded-lg hover:border-blue-400">
      ...
      {step === 2 && selectedTextbook && (
        <form onSubmit={handleSubmit} className="space-y-5">
          <p className="text-gray-600">Step 2: Set price and condition</p>
          <div className="p-4 bg-gray-50 rounded-lg">
            <div className="font-medium">{selectedTextbook.title}</div>
            ...
            <div className="flex gap-2">
              {[1, 2, 3, 4, 5].map(v => (
                <button key={v} type="button"
                  ...
                  className={`flex-1 py-2 rounded-lg border text-sm ${condition === v ? 'border-blue-500 bg-blue-50' : 'border-gray-200'}`}>
                  <div className="text-lg">{'★'.repeat(v)}{'☆'.repeat(5 - v)}</div>
