    ...
    <div>
      ...
      <section className="mb-8">
        <div className="flex items-center justify-between mb-3">
          ...
          <Link to="/search" className="neo-btn px-3 py-1 text-xs font-bold text-orange-500">全部 →</Link>
        </div>
        <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-9 gap-2">
          {categories.map(cat => (
            <Link key={cat.id} to={`/search?category=${cat.id}`}
              className="group flex flex-col items-center neo-card p-2.5 sm:p-3 hover:shadow-[12px_12px_24px_#e0dbd6,-12px_-12px_24px_#ffffff] transition-all duration-200 hover:-translate-y-0.5">
      ...
      <section>
        <div className="flex items-center justify-between mb-3">
          ...
          <Link to="/search" className="neo-btn px-3 py-1 text-xs font-bold text-orange-500">查看更多 →</Link>
        </div>
        {loading ? (
          ...
          <div className="text-center py-16 neo-card">
            ...
            <Link to="/create" className="neo-btn-primary px-5 py-2 mt-3 inline-block">🚀 立即发布</Link>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
