    ...
    <div className="max-w-md mx-auto mt-8 sm:mt-14">
      ...
      <div className="neo-card p-6 sm:p-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-sm font-bold text-gray-600 mb-1.5">😎 昵称</label>
          ...
          <div>
            <label className="block text-sm font-bold text-gray-600 mb-1.5">🔒 密码</label>
            <input type="password" placeholder="至少6位" value={password}
              onChange={e => setPassword(e.target.value)}
              className="w-full neo-inset px-4 py-3 text-sm font-bold text-gray-700" required />
          </div>
          <button type="submit" className="w-full neo-btn-primary py-3.5 text-sm">
            🎉 注册
          </button>
        </form>
        <p className="mt-6 text-center text-gray-400 text-sm font-bold">
          已有账户？ <Link to="/login" className="font-bold" style={{color:'#ff7b3d'}}>立即登录</Link>
        </p>
      </div>
    </div>
  );
